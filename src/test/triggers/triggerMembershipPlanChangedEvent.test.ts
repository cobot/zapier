...
describe("triggerMembershipPlanChangedEvent", () => {
  it("creates new webhook through CM API upon subscribe", async () => {
    const bundle = prepareMocksForWebhookSubscribeTest(
      triggerMembershipPlanChangedEvent.key,
    );
    const subscribe = (
      App.triggers[triggerMembershipPlanChangedEvent.key] as HookTrigger
...
`);
  ...
  it("lists recent memberships", async () => {
    ...
    const listMemberships =
      triggerMembershipPlanChangedEvent.operation.performList;
  ...
  it("triggers on membership plan changed", async () => {
    const bundle = prepareBundle({
    ...
    const results = await appTester(
      triggerMembershipPlanChangedEvent.operation.perform as any,
      bundle as any,
    );
