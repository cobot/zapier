...
describe("triggerMembershipUncanceled", () => {
  it("creates new webhook through CM API upon subscribe", async () => {
    const bundle = prepareMocksForWebhookSubscribeTest(
      triggerMembershipUncanceled.key,
    );
    const subscribe = (
      App.triggers[triggerMembershipUncanceled.key] as HookTrigger
...
`);
  ...
  it("lists recent memberships", async () => {
    ...
    const listMemberships = triggerMembershipUncanceled.operation.performList;
  ...
  it("triggers on membership uncanceled", async () => {
    const bundle = prepareBundle({
    ...
    const results = await appTester(
      triggerMembershipUncanceled.operation.perform as any,
      bundle as any,
    );
