...
describe("triggerMembershipReactivated", () => {
  it("creates new webhook through CM API upon subscribe", async () => {
    const bundle = prepareMocksForWebhookSubscribeTest(
      triggerMembershipReactivated.key,
    );
    const subscribe = (
      App.triggers[triggerMembershipReactivated.key] as HookTrigger
...
`);
  ...
  it("lists recent memberships", async () => {
    ...
    const listMemberships = triggerMembershipReactivated.operation.performList;
  ...
  it("triggers on membership reactivated", async () => {
    const bundle = prepareBundle({
    ...
    const results = await appTester(
      triggerMembershipReactivated.operation.perform as any,
      bundle as any,
    );
