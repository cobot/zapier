...
import trigger from "../../triggers/triggerCreatedMembership";
...
describe("triggerCreatedMembership", () => {
  it("creates new webhook through CM API upon subscribe", async () => {
    const bundle = prepareMocksForWebhookSubscribeTest(
      triggerCreatedMembership.key,
    );
    const subscribe = (
      App.triggers[triggerCreatedMembership.key] as HookTrigger
...
`);
  ...
  it("triggers on membership created", async () => {
    const bundle = prepareBundle({
    ...
    const results = await appTester(
      triggerCreatedMembership.operation.perform as any,
      bundle as any,
    );
