...
const dropInPass: DropInPassApiResponse = {
  id: "1",
  attributes: {
    name: "Day Pass",
    validOn: "2012-04-12",
...
`);
  ...
  it("triggers when drop in pass is purchased", async () => {
    const bundle = prepareBundle({
      url: "https://api.cobot.me/drop_in_passes/1",
    });
    const scope = nock("https://api.cobot.me");
    scope.get("/drop_in_passes/1").reply(200, { data: dropInPass });
    const results = await appTester(
