...
describe("triggerMembershipUpdated", () => {
  it("creates new webhook through CM API upon subscribe", async () => {
    const bundle = prepareMocksForWebhookSubscribeTest(
      triggerMembershipUpdated.key,
    );
    const subscribe = (
      App.triggers[triggerMembershipUpdated.key] as HookTrigger
...
`);
  ...
  it("lists recent memberships", async () => {
    ...
    const listMemberships = triggerMembershipUpdated.operation.performList;
  ...
  it("triggers on membership updated", async () => {
    const bundle = prepareBundle({
    ...
    const results = await appTester(
      triggerMembershipUpdated.operation.perform as any,
      bundle as any,
    );
