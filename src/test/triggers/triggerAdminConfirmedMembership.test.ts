...
describe("triggerAdminConfirmedMembership", () => {
  it("creates new webhook through CM API upon subscribe", async () => {
    const bundle = prepareMocksForWebhookSubscribeTest(
      triggerAdminConfirmedMembership.key,
    );
    const subscribe = (
      App.triggers[triggerAdminConfirmedMembership.key] as HookTrigger
...
`);
  ...
  it("lists recent memberships", async () => {
    ...
    const listMemberships =
      triggerAdminConfirmedMembership.operation.performList;
  ...
  it("triggers on admin confirmed membership", async () => {
    const bundle = prepareBundle({
    ...
    const results = await appTester(
      triggerAdminConfirmedMembership.operation.perform as any,
      bundle as any,
    );
