...
/**
 * Normalizes a booking webhook payload to match BookingApiResponse shape.
 * Cobot's deleted_booking webhook may send flat fields (e.g. resource_name)
 * instead of the nested API response shape (e.g. resource: { name, id }).
 */
export function normalizeBookingPayload(raw: any): BookingApiResponse {
  return {
    id: raw.id,
    from: raw.from,
    to: raw.to,
    ...
    attendeesMessage: raw.attendeesMessage ?? raw.attendees_message,
  };
}

/**
 * Normalizes a membership webhook payload to match MembershipApiResponse shape.
 * Adds defensive defaults for nested fields that may be missing in webhook payloads.
 */
export function normalizeMembershipPayload(raw: any): MembershipApiResponse {
  return {
    id: raw.id,
    name: raw.name ?? "",
