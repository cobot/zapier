...
import { SubscribeBundleInputType, SubscribePayloadType } from "../types/subscribeType"
import { InputData as ActivityInputData } from "../creates/activity"
import { get } from "lodash"

export const subscribeHook = async (z: ZObject, bundle: KontentBundle<SubscribeBundleInputType>, data: SubscribePayloadType) => {
...
export const unsubscribeHook = async (z: ZObject, bundle: KontentBundle<SubscribeBundleInputType>, subscribeId: string) => {
...
export const listRecentBookings = async (z: ZObject, bundle: KontentBundle<SubscribeBundleInputType>) => {
...
export const listRecentExternalBookings = async (z: ZObject, bundle: KontentBundle<SubscribeBundleInputType>) => {
  try {
    const subdomain = bundle.inputData.subdomain
    const userV2 = await getUserDetailV2(z)
    var admin = userV2.included.find(x => get(x, 'attributes.subdomain', '') === subdomain)
    if (admin) {
...
export const getExternalBookings = async (z: ZObject, id:string) => {
  ...
  try {
    ...
    const booking = get(response.data, 'data.relationships.externalBooking.data')
    if (booking) {
      const response = await z.request({
...
export const createActivity = async (z: ZObject, bundle: KontentBundle<ActivityInputData>) => {
  const response = await z.request({
    method: 'POST',
...
export const getDateRange = () => {
  var now = new Date()
  var lastMonth = new Date(new Date(now).setMonth(now.getMonth() - 1)).toISOString();
  var nextWeek = new Date(new Date(now).setDate(now.getDate() + 7)).toISOString();
